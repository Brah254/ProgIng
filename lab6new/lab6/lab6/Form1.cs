...
namespace lab6
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                textBox6.SelectionStart = hScrollBar1.Value;
        ...
        {
            int rows = int.Parse(textBox4.Text);
            int cols = int.Parse(textBox5.Text);

            if (!int.TryParse(textBox4.Text, out rows) || !int.TryParse(textBox5.Text, out cols) || rows <= 0 || cols <= 0)
            {
                MessageBox.Show("Invalid submatrix dimensions. Please enter positive integer values.");
                return;
            }

            if (matrix.Count < rows || matrix[0].Count < cols)
            {
                MessageBox.Show("Submatrix dimensions exceed the size of the main matrix.");
                return;
            }
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    ...
                    if (j < cols - 1) // Add tab except for the last element in a row
                        result.Append("\t\t");
                }
